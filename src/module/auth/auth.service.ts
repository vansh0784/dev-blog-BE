...
export class AuthService {
    ...
    async login(data: SignInInput): Promise<AuthResponse> {
        ...
        if (existingUser && !(await compare(data.password, existingUser.password))) throw new UnauthorizedException('Invalid Credentials!');

        const token = this.generateToken({ userId: existingUser.id, email: existingUser.email, role: existingUser.role });
